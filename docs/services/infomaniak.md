...
3. Store the Infomaniak account credentials in **Bitwarden cloud**.
4. Purchase a kSuite plan with Mail, Files, and Drive services.
...
3. Scan QR code with **Bitwarden Authenticator**
4. Store the TOTP seed and recovery code in **Bitwarden cloud** (not Vaultwarden)
...
1. Instruct each family member to enable TOTP when they first log in:
   Manager → avatar → Security → Two-factor authentication
2. Recommend **Bitwarden Authenticator** — it syncs TOTP seeds to Vaultwarden once deployed
3. Verify with each member that MFA is active before the DNS cutover
4. Store backup codes in **Vaultwarden** per user (family users → Vaultwarden, not Bitwarden cloud)
