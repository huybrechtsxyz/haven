...
| Backups             | Two-tier backups          | Hetzner + Infomaniak                                              | **Tier 1:** Hearth + Forge system state via BorgBackup → Storage Box BX11 (daily, encrypted). **Tier 2:** Storage Box + S3 buckets (`haven-photos`, `haven-media`, `haven-docs`) synced to dedicated Infomaniak kDrive 3 TB once a day via rclone — offsite cross-provider copy |
...
Tier 2 — Daily offsite sync (rclone, ~03:00 UTC)
  ...
  S3 haven-docs                              ──┘    cross-provider, Swiss datacentre
```
