...
| `gaming`    | [services/foundryvtt.md](../services/foundryvtt.md), [services/rpgkeeper.md](../services/rpgkeeper.md)                                          | Foundry Virtual Tabletop (VTT server) + RPGKeeper (character sheet manager), own local-path PVCs — no Storage Box mount needed (see note below) |
...
- **Grimoire** → `/mnt/haven-data-docs/ttrpg` (read-only, maps/tokens/audio/models) **and** `/mnt/haven-data-docs/books/ttrpg` (read-only, bind-mounted at `/library/books` — only the TTRPG subset of Kavita's `books/` tree, not the whole ~20K-book library)
...
- `system`/`immich`/`media`/`documents` namespaces are all deployed and running in production (Immich, Jellyfin, Nextcloud, Kavita all healthy, HTTPS working). The `documents` namespace's SMB storage migration (removing the old `rclone-mount` module, re-pointing Nextcloud/Kavita hostPaths to `/mnt/haven-data-docs`, wiring `STORAGEBOX_DATA_DOCS_PASSWORD`) is complete and confirmed end-to-end (2026-08-30): a file uploaded via Nextcloud's web UI landed on the real Storage Box and was picked up by Kavita's library scan. The `haven-docs` S3 bucket was decommissioned; `haven-data`'s `docs` sub-account is the replacement.
- `finance` namespace (Firefly III) added 2026-09-06 — config authored and validated, **not yet deployed/tested live**. First use of Traefik forwardAuth -> Authentik SSO in this repo (every other app uses native OIDC); see [services/firefly.md](../services/firefly.md) for the full design + open risks before running `33 - Forge - Deploy` against it.
- Jellyfin's SSO plugin requires the server's `image.tag` to stay at or above the plugin's `targetAbi` (see [jellyfin.md](../services/jellyfin.md#what-gets-deployed)) — Jellyfin silently disables plugins that fail this check rather than erroring, so a stale/unset image tag is an easy way to reintroduce a silent SSO outage. Check `docs/services/jellyfin.md` for current SSO status before assuming it's working.

- **CIFS Unix Extensions silently override client permission options (2026-08-30, fixed):** Hetzner Storage Box's CIFS server negotiates the "Unix Extensions" capability, which makes the *server's* stored file mode/owner authoritative — the client-side `file_mode`/`dir_mode` cifs mount options are silently ignored while this is active. This caused a long-lived bug: directories were created at `0755 root:root` by an early Ansible run, and no combination of `file_mode=0777,dir_mode=0777` mount options in later runs changed that, because Unix Extensions kept reporting the original stored mode. The fix is the `nounix` mount option in `deploy/ansible-forge/forge-init.yml`'s docs-share mount task — it disables Unix Extensions client-side, falling back to standard SMB semantics where `file_mode`/`dir_mode`/`uid`/`gid` become the sole source of truth regardless of what's stored server-side. **Applies to any future Storage Box SMB mount that needs writes from a non-root container user** (e.g. Nextcloud's `www-data`) — add `nounix` alongside `file_mode`/`dir_mode` from the start rather than debugging the same symptom again.

- **Stale pod bind-mounts after a host-level remount (2026-08-30):** after fixing the mount options above and re-running `31 - Forge - Init`, the Nextcloud/Kavita pods still showed the old (pre-fix) mount when checked from inside the pod — because a pod's hostPath bind-mount is a snapshot taken at container start, and doesn't automatically pick up a host-side remount that happens afterward. Fix: `kubectl rollout restart deployment/<name> -n documents` after any host-level mount change to this path.
