...
- `system` (`rclone-mount`, `cert-manager`, `cert-manager-issuers`), `immich`, and `media` (Jellyfin) namespaces are active in `config/stack/workspace.yaml`; `system`/`immich` verified in production including working HTTPS (Immich on `letsencrypt-prod`), `media` just activated and not yet confirmed deployed/healthy. `documents` is still pruned out, to be activated next.
