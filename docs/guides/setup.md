...
| `HETZNER_PRIVATE_KEY`     | Hetzner SSH deployment key (private, ed25519) | **Dual-use, same reason as above** — also stored in Infisical (used by Terraform for the `hcloud_ssh_key` resource), but every Ansible-based workflow (Hearth/Forge init/config/deploy, `backup-haven.yml`) writes it directly to `~/.ssh/haven_ed25519` on the runner via `secrets.HETZNER_PRIVATE_KEY`, bypassing Infisical entirely. Must be set in **both** places with the same value. |
